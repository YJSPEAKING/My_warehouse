...
void* thread_func(void *p)
{
    ...
    int ret=pthread_cond_timedwait(&d->cond,&d->mutex,&abstime);//wait的前后有加解锁包着
    pthread_mutex_unlock(&d->mutex);
    printf("I am child,wakeup,timedwait ret=%d start work\n",ret);
    pthread_exit(NULL);
}
int main()
{
    ...
    sleep(1);
    //负责唤醒
    ret=pthread_cond_signal(&d.cond);//像一个脉冲，让条件一瞬间成立，如果成立后才去排队，得不到唤醒
